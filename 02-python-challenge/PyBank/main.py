...
def main():
    ...
    net_total_amount = 0
    average_change = 0
    greatest_increase = float('-inf')   # sets the lower limit for the revenue change
    greatest_decrease = float('inf')    # sets the upper limit for the revenue change
    greatest_increase_date = 'Mon-Year'
    greatest_decrease_date = 'Mon-Year'
    prev_revenue = None     # this enebles us to skip the first revenue row when computing the revenue change
    revenue_change_ls = []
    ...
    with open(csv_path, newline='') as f: #quivalent to f = open(csv_reader, 'r'), f('close')
        ...
        for row in reader:
            ...
            if prev_revenue is not None:
                ...
                # append to the revenue change list
